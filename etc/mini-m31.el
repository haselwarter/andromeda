    ...
    (cl-flet ((f (cl-case dir ('forward 'looking-at)
      ...
      (cond
       ...
       ((f
         ...
         (concat "^\\(:?" let-rec-rx "\\)")) (if (m31--llet-heuristic) "LLET" "TLET"))
       ((f "^handle") (if (m31--local-handle-heuristic) "LHANDLE" "THANDLE"))
       ((f let-rec-rx)      "LLET")
       ((f m31-pvar-rx)     "NAME")
       ((f m31--name-re)
  ...
  (smie-prec2->grammar
   (smie-bnf->prec2
    ...
    '((topdirs  (topdirs "CONSTANT" topdirs)
                ...
                (topdirs "THANDLE" patterns "END" topdirs)
                ;; (topdirs "THANDLE" topdirs)
      ...
      (term     (ty_term)
                ("LHANDLE" term "WITH" patterns "END")
