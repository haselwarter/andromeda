...
and 'a term' =
  ...
  | ComputeEquation of ('a term * 'a term) list * 'a term list * 'a list * 'a term
...
and toplevel' =
  ...
  | OpaqueDefine of name * name term (* define x := e *)
...
and string_of_term string_of_var (term,_) =
  ...
  | ComputeEquation(eqs1, terms2, xs3, term4) ->
      paren "ComputeEquation" [
                  paren "" (List.map (fun (l,r) -> paren "" [recurse l; recurse r]) eqs1);
                  paren "" (List.map recurse terms2);
                  paren "" (List.map string_of_var xs3);
                  recurse term4]
